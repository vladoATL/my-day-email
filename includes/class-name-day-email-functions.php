<?php
	...
	function namedayemail_create($user, $istest = false){
		$options = get_option('namedayemail_options'); 
		$subject_user = $options['subject'];
		$html_body = $options['namedayemail_body'];
		$from_name = $options['from_name'];
		$from_address = $options['from_address'];	
		$header  = $options['header'];
				
		if ($istest == true) {
			$headers_user   = namedayemail_headers($from_name, $from_address,"", "", true);
			$email = $options['bcc_address'];
		} else {
			$headers_user   = namedayemail_headers($from_name, $from_address,"", "", false);
			$email = $user->user_email;
		}

		if (str_contains($html_body, '{coupon}') && (array)$user) { 			
		    $coupon = namedayemail_get_unique_coupon($user);
		    if (empty($coupon))	{
		    	namedayemail_add_log("No available coupon to create.");
		    	return;
		    }
			$html_body = str_replace('{coupon}',$coupon,$html_body);
		}		

		$html_body = namedayemail_replace_placeholders($html_body, $user, $options);
		$subject_user = namedayemail_replace_placeholders($subject_user, $user, $options); 
		...
		if ((!str_contains(get_home_url(), 'test') && !str_contains(get_home_url(), 'stage') && $options['test'] != 1) || $istest == true){
			if (is_email($email)) {
				...
				if ($options['wc_template'] == 1) {
					namedayemail_send_wc_email_html($subject_user, $email, $html_body, $header);
				} else {
					$sendmail_user = wp_mail( $email, $subject_user, $html_body, $headers_user );
				}			
				
				if ($istest == true) {
					 namedayemail_add_log("Test email sent to" . ': ' . $email ) ;
				} else {
					namedayemail_add_log("Email sent to" . ': ' . $email ) ;
				}
			} else {
				namedayemail_add_log("Trying to send to incorrect or missing email address"  . ': ' . $email ) ;
			}
		} else {
			...
			if ($options['wc_template'] == 1) {
				namedayemail_send_wc_email_html($subject_user, $admin_email, $html_body, $header);
			} else {
				$sendmail_user = wp_mail( $admin_email, $subject_user, $html_body, $headers_user );
			}									
			namedayemail_add_log("Email has been sent as Test to"  . ' ' . $admin_email . " instead of to " . $email) ;
		}
	}

	function namedayemail_replace_placeholders($content, $user, $options) {
	...
	function namedayemail_add_log($entry){
		$options = get_option('namedayemail_options');
		if ($options['enable_logs'] == "1") {
			...
			$log = $options['logs'];
			if (empty($options)){
				add_option( 'namedayemail_logs', array('logs'	=>	$entry) );
			} else {
				update_option( 'namedayemail_logs',array('logs'	=>	$log . "\n" .  $entry) );
			}
		}
	}

	function namedayemail_headers($from_name, $from_address, $email_cc, $email_bcc, $istest = false) {
		$headers_user   = array();
	...
	function namedayemail_get_unique_coupon($user){			
		global $wpdb;
		$options = get_option('namedayemail_options'); 
		...
		$description = namedayemail_replace_placeholders($description, $user, $options);
		...
		$coupon = array(
		  ...
		  //namedayemail_add_log("wp_insert_post" . ': '  . $user->ID  . ' ' . implode($coupon) ) ;          
		...
		$cat_id = namedayemail_coupon_category($new_coupon_id, $options['category']);	
		return $generated_code;
	}	
	
	function namedayemail_coupon_category($new_coupon_id, $cat_slug){
				global $wpdb;
				if ( ! is_plugin_active( 'advanced-coupons-for-woocommerce-free/advanced-coupons-for-woocommerce-free.php' ) ) return 0;					
					$term_id = namedayemail_get_cat_slug_id($cat_slug) ;
					if (! empty($term_id)) {
	...
	function namedayemail_get_cat_slug_id($cat_slug){
			global $wpdb;
	...
	function namedayemail_send_wc_email_html($subject, $recipient, $body, $heading = false ) {
		$template = 'emails/nameday.php';
		$mailer = WC()->mailer();
		$options = get_option('namedayemail_options'); 		
		$headers = namedayemail_headers($options['from_name'], $options['from_address'], $options['cc_address'], $options['bcc_address'] );
	...
	function time_utc($dateTime){				
		...
		if (!$newDateTime instanceof DateTime) return "";
